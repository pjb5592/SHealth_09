# SHealth BMI 프로젝트 — 리팩토링 우선 프롬프트 (수정, 2026-05-20)

> 원본: `prompt_리펙토링 우선 진행.md`  
> 목적: **분석·1차 리팩토링 후 테스트 계획·TDD(4~5단계)**를 수행하고, **결함·기능·2차 로드맵(6~8단계)** 및 QA(9~12단계)로 이어지는 워크플로우
...
3. **0~3단계**(분석·1차 리팩토링): `cmake --build build` 성공 + (가능하면) `SHealthBMI` 수동 실행으로 동작 확인. `ctest` Green은 **5단계 이후** 완료 조건.
4. **4~5단계**(테스트 계획·TDD): `cmake --build build && ctest` Green을 완료 조건으로 둔다.
5. **6~12단계**(결함·기능·로드맵·Golden·QA): 5단계 Green을 유지하며 진행한다.
6. 세션 종료 시 **후처리 프롬프트**를 한 번 실행한다.
...
| 워크플로우 | **수정 순서** — 코드 품질·1차 리팩토링 → 테스트 계획·TDD → 결함·기능·2차 로드맵 → QA |
...
| 리팩토링 | 동작 변경 최소화 · 매직 넘버 상수화 · 함수·클래스 분리 — **5단계 Green 이후** 추가 리팩토링 시 테스트 유지 |
| 검증 명령 | `mkdir build && cd build && cmake .. && cmake --build .` · 5단계~: `&& ctest` |
...
[C] SHealth BMI C++17 프로젝트 — **리팩토링 우선(수정)** 워크플로우용 Cursor 규칙
[T] 프로젝트 루트 `.cursorrules` 작성
    - 기술 스택: C++17, CMake, Google Test, shealth_lib
    - 도메인: BMI 공식, 4단계 분류 기준, 연령대 구간, 체중 0 평균 보정
    - 워크플로우: 분석 → 1차 리팩토링 → 테스트 계획·TDD → 결함·기능 → 2차 로드맵
    - 리팩토링: 매직 넘버 상수화, SRP·함수 추출, 최소 변경, 동작 보존
    - 테스트: 4~5단계에서 집중 — Given-When-Then, 경계값, 단일 조건 per TEST
    - README Activities 1~5 순서를 **수정 워크플로우** 관점으로 재해석
...
**완료 기준:** `docs/defect_list.md` + 확인된 결함 수정 후 `ctest` Green
...
```
[P] 프로젝트 문서·배포 담당
[C] 방금 완료한 단계 번호 N, 산출물 경로, 워크플로우: 리팩토링 우선(수정)
[T] 아래 순서로 실행
...
| # | 단계 | 핵심 지시 | 검증 |
|---|------|-----------|------|
| 0 | 프로젝트 규칙 | `.cursorrules` 작성 | 파일 생성 |
| 1 | 요구사항 분석 | README → requirements_analysis.md | 문서만 |
| 2 | 코드 품질 분석 | SOLID·스멜·우선순위 1~5 | 문서만 |
| 3 | 1차 리팩토링 | SRP·DRY·Magic Number, 동작 보존 | 빌드 + SHealthBMI 출력 |
| 4 | 테스트 계획 | TDD 4영역·경계값·Fixture 전략 | test_plan.md |
| 5 | TDD 구현 | 실패 테스트 → Green | ctest Green |
| 6 | 결함 분석 | 정적 분석 + defect_list + TC 매핑 | ctest Green |
| 7 | 기능 개선 | README Activities 4, SRP·API 추가 | ctest Green |
| 8 | 2차 로드맵 | STL·테이블 드리븐 Phase 계획 | 문서(변경 시 ctest) |
| 9 | Golden Master | SHealthBMI 출력 회귀 (선택) | ctest Green |
| 10 | 결함 관리 | 프로세스·템플릿 문서 | defect_report.md |
| 11 | 설계 다이어그램 | Mermaid 클래스 다이어그램 (선택) | architecture.md |
| 12 | QA 종합 | Before/After·커버리지·회고 | qa_final_report.md |

---

## 원본·테스트 우선 vs 수정 순서 비교

| 구분 | 테스트 우선 | 리팩토링 우선 (원본) | **수정 (본 문서)** |
|------|-------------|---------------------|-------------------|
| 테스트 계획·TDD | 3~4단계 | 7~8단계 | **4~5단계** |
| 1차 리팩토링 | 5단계 | 3단계 | **3단계** |
| 결함 분석 | 6단계 | 4단계 | **6단계** |
| 기능 개선 | 7단계 | 5단계 | **7단계** |
| 2차 로드맵 | 9단계 | 6단계 | **8단계** |
| 초반 검증 | ctest Green | 빌드 + 수동 출력 | 빌드 + 수동 (0~3) |
| 중반 검증 | ctest Green | — | **ctest Green (4~5)** |
| 후반 검증 | ctest Green | ctest + Golden | ctest Green 유지 |
...
| 개선 항목 | 내용 |
|-----------|------|
| 단계 재배치 | 1차 리팩토링 직후 **테스트 계획·TDD(4~5)** → 결함·기능·로드맵(6~8) |
| 검증 기준 | 0~3: 빌드·수동 / 4~5: ctest Green / 6~12: Green 유지 |
| 의존성 갱신 | mermaid·`@` 참조·완료 기준을 수정 순서에 맞게 조정 |
| 결함·테스트 | 5단계 TDD 후 6단계에서 defect_list·회귀 TC 매핑 |
...
```
SHealth BMI C++17 **리팩토링 우선(수정)** QA 프로젝트를 아래 순서로 진행해줘.
공통 제약: BMI 공식·4분류 기준·연령대 [20,30,…,70) 구간, weight=0 평균 보정.
0~3단계는 빌드+수동 출력, 4~5단계부터 ctest Green, 6~12단계는 Green 유지.
...
4) 테스트 계획 → docs/test_plan.md (1차 리팩토링 후 구조 반영)
5) TDD: test_plan 기준 실패 테스트 → Green
6) 결함 분석·docs/defect_list.md + 수정·회귀 TC 매핑
7) README 기능 개선 (SRP, height=0, 목록·전체 비율) + ctest Green
8) 2차 리팩토링 로드맵 → docs/refactoring_plan.md
